...
Beginning in September, 2014, I began to archive price data on illicit drugs from deep web marketplaces, such as the Silk Road. This document describes the process with which I cleaned the data and the process through which I construct price indices.
...
The file `all_data.RData` contains the raw, unprocessed data. It consists of two parts, `listings` and `listings_prices`. `listings` has a complete list of all listings that were archived, along with the site of origin, and a (sometimes missing) classification of the type of drug. `listings_prices` contains a complete list of what I call 'price points'. A price point is a record of a particular price that the listing was offered for at a particular time. It contains the number of items offered at that time, some measurement of the amount of drug per item, the price, and the parent listing. 
